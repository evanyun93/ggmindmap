...
async function sendEmailVerificationCode(toEmail, verificationCode) {
  const mailOptions = {
    from: process.env.SMTP_FROM || '"MindMap" <noreply@mindmap.app>', 
    to: toEmail,
    subject: '[MindMap] 이메일 인증번호',
    text: `요청하신 이메일 인증번호는 [ ${verificationCode} ] 입니다. 이 인증번호는 10분간 유효합니다.`,
    html: `
      <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 10px;">
        <h2 style="color: #333; text-align: center;">MindMap 이메일 인증</h2>
        <p style="font-size: 16px; color: #555;">이메일 등록/변경을 위한 인증번호입니다.</p>
        <div style="background-color: #f4f4f4; padding: 15px; text-align: center; border-radius: 5px; margin: 20px 0;">
          <span style="font-size: 24px; font-weight: bold; letter-spacing: 5px; color: #007bff;">${verificationCode}</span>
        </div>
        <p style="font-size: 14px; color: #777;">이 인증번호는 10분간 유효합니다. 본인이 요청하지 않은 경우 이 메일을 무시해 주세요.</p>
